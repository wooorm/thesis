...
spaces\footnote{For example, Kwnl counts words using this method
...
Many implementations exist for this task\footnote{For example,
  \textcite{thinkroth/sentimental-source-code},
  \textcite{mileszim/sediment-source-code},
  \textcite{thisandagain/sentiment-source-code}.} on the web platform,
...
available\footnote{For example,
  \textcite{jbrooksuk/node-summary-source-code} on the web,
  \textcite{MojoJolo/textteaser-source-code} in Scala.}.
...
use a whole different approach to sentence tokenisation (see the next
section).

\subsubsection{Using Corpora for \gls{nlp}}\label{using-corpora-for}
...
\subsubsection{Using a web \acrshort{api}}\label{using-a-web}
...
\subsubsection{Syntax: \gls{nlcst}}\label{syntax}
...
\subsubsection{Parser: Parse-latin}\label{parser-parse-latin}
...
\subsubsection{Object Model: \gls{textom}}\label{object-model}
...
\subsubsection{Natural Language System:
...
\subsubsection{\Gls{api} Design}\label{design-1}
...
side on Node.js, npm---the default package manager for the platform---is
...
available for download through Git and GitHub, npm, Bower, and Component
were used.
...
\subsection{Reception}\label{reception}
