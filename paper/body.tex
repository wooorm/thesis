...
These tasks all part of \emph{information extraction}: the act of finding
...
\item
  ...
  \end{enumerate}
\item
  What are the target audiences use cases for an implementation?
  \begin{enumerate}
...
The proposal solves the creation of natural language by default (use case
  \ref{list:use-case:2}), but these plugins solve several others.
The developed plugins included implementations for:
...
\item Detecting grammatical units, solving use case
...
These plugins listed and the other plugins solve several use cases of the
  target audience (§~\ref{use-cases}).
The unsolved use cases can be solved using the plugin mechanism provided by
  \emph{Retext}.
...
Neither is language recognition (use case \ref{list:use-case:6}),
...
\gls{nlp} covers many different tasks.
...
\subsection{The target audiences use cases}\label{q-use-cases}
...
Although the \emph{how} was answered by the \emph{research question}, the
