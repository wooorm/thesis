...
[^implementations-5]: For example, Knwl[@loadfive/knwl-source-code] counts words using this method.

[^implementations-6]: For example, Natural's [@NaturalNode/natural-source-code] tokenisers.
...
[^implementations-13]:
    A simple rule-based sentence tokeniser could be implemented[@attivio.com-doing-things-with-sentences] as follows:
...
[^implementations-14]: Currently, almost no technologies exist for storing large datasets in a browser. The only exception to the rule is the \gls{html5} File System \gls{api}, but it was recently deprecated[@w3.org-filesystem-dead].
...
The definitions of exposed nodes were heavily based on other specifications of syntax trees for manipulation on the web platform, such as CSS (aptly named for the \gls{css} language)[@reworkcss/css-source-code] and the Mozilla JavaScript \gls{ast} (for ECMAScript)[@mozilla.org-spidermonkey-parser_api]. Both implementations are widely used. CSS by Rework[@reworkcss/rework-source-code], and Mozilla JavaScript \gls{ast} by Esprima[@ariya/esprima-source-code], Acorn[@marijnh/acorn-source-code], and Escodegen[@constellation/escodegen-source-code].
...
To reach the target audience and validate its use, the software was developed as open source software---software that is free for all to use and redistribute. All projects were licensed under The \gls{MIT} License[@opensource.org-licenses-mit], a license which provides rights for others to use, copy, modify, merge, publish, distribute, sublicense, and/or sell copies of these projects.
